...
$.SvgCanvas = function (container, config) {
  ...
  var svgdoc = container && container.ownerDocument;
  ...
  // 浮动显示当前缩放级别 (1 = 100%, .5 = 50%, etc)
  var current_zoom = 1;

  // pointer to current group (for in-group editing)
  var current_group = null;

  // Object 其中包含当前选定样式的数据
  var all_properties = {
    shape: {
  ...
  // 根据给定的对象键/值创建一个新的SVG元素，并将其添加到当前层
  // 该元素将在返回之前通过cleanupElement运行
  //
  // Parameters:
  // data - Object with the following keys/values:
  // * element - 标记要创建的SVG元素的名称
  // * attr - Object 属性键值赋给新元素
  // * curStyles - 指示应该首先应用当前样式属性的布尔值
  //
  // Returns: The new element
  var addSvgElementFromJson = this.addSvgElementFromJson = function (data) {
    var shape = svgedit.utilities.getElem(data.attr.id);
    // 如果shape是一个路径，但我们需要创建一个矩形/椭圆，然后删除路径
  ...
  var addSvgElementFromJsonTwo = this.addSvgElementFromJson = function (data) {
    var shape = svgedit.utilities.getElem(data.attr.id);
  ...
  // 发送一个实现ElementContainer接口的对象 (see units.js)
  svgedit.units.init({
    ...
    var mouseUp = function (evt) {
      ...
      switch (current_mode) {
        ...
        case "fhpath":
          ...
          break;
        case "xpc":
          // 检查路径是否包含至少2个点;退化的一点路径
          // causes problems.
          // Webkit忽略了我们如何用逗号设置points属性并使用空格
          // to separate all coordinates, see https://bugs.webkit.org/show_bug.cgi?id=29870
          var coords = element.getAttribute('points');
          var commaIndex = coords.indexOf(',');
          if (commaIndex >= 0) {
          ...
          if (keep) {
            element = pathActions.smoothPolylineIntoPath(element, 'xpc');
          }
          break;
        case "line":
  ...
  var pathActions = canvas.pathActions = function () {
    ...
    // 这个函数将折线(由fh_path工具创建)转换为
    // 路径元素，每三个线段转换为一个bezier
    // 曲线试图使徒手平滑
    var smoothPolylineIntoPath = function (element, key) {
      var points = element.points;
      var N = points.numberOfItems;
      if (N >= 4) {
        // 循环通过每3点，并转换为三次贝塞尔曲线段
        // 
        // 注意:这是作弊，这意味着每3个点都有可能成为一个角，而不是以平等的方式对待每一个点。一般来说，这种技术看起来不是很好
        ...
        element = '';
        if (key) {
          element = addSvgElementFromJsonTwo({
            "element": "path",
            "curStyles": true,
            "attr": {
              "id": getId(),
  ...
  this.setLayerVisibility = function (layername, bVisible) {
    ...
    if (layer) {
      ...
      addCommandToHistory(new ChangeElementCommand(layer, {
        'display': oldDisplay
      }, 'Layer Visibility'));
    } else {
      return false;
  ...
  this.setGroupTitle = function (val) {
    ...
    if (!val.length) {
      ...
      var title = ts[0];
      batchCmd.addSubCommand(new ChangeElementCommand(title, {
  ...
  this.setDocumentTitle = function (newtitle) {
    ...
    batchCmd.addSubCommand(new ChangeElementCommand(doc_title, {
      '#text': old_title
    }));
    addCommandToHistory(batchCmd);
  }
  ...
  this.setResolution = function (x, y) {
    ...
    if (x !== w || y !== h) {
      ...
      this.contentW = x;
      this.contentH = y;
      batchCmd.addSubCommand(new ChangeElementCommand(svgcontent, {
        "width": w,
        "height": h
      }));
  ...
  this.setMode = function (name) {
    ...
    if (name !== 'select') {
      cur_properties.stroke_width = 1;
      cur_properties.stroke = "#000";
    }
    current_mode = name;
  };
  ...
  this.setTextContent = function (val) {
    ...
    const textPath = selected.querySelector("textPath") ? selected.querySelector("textPath") : null;
    ...
    if (canvas.elementsAreSame(selectedElements) && selectedElements[0].tagName == "rect") {
      var assign_rr = function (selected) {
        ...
        if (r != val) {
          ...
          addCommandToHistory(new ChangeElementCommand(selected, {
            "rx": r,
            "ry": r
          }, "Radius"));
          call("changed", [selected]);
